    ...
    <div className="p-6 max-w-6xl mx-auto text-gray-200">
      <div className="flex justify-between items-end mb-8 border-b border-gray-800 pb-6">
        <div>
          ...
          <p className="text-gray-400">Manage your credits balance and view translation usage.</p>
        </div>
        <Link href="/pricing" target="_blank" className="px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm transition text-gray-300 border border-gray-700">
          View Pricing
        </Link>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-8">
        ...
        {/* Wallet Balance */}
        <div className="md:col-span-1 bg-gradient-to-br from-gray-900 to-[#111827] border border-gray-800 rounded-2xl p-6 shadow-xl relative overflow-hidden">
          ...
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2">Credits Balance</h2>
          {loading ? (
            <div className="animate-pulse h-12 w-32 bg-gray-800 rounded mb-4" />
          ) : (
            <div className="flex items-baseline mb-6">
              <span className="text-5xl font-extrabold text-white tracking-tight">
                {Number(wallet?.balance || 0).toFixed(2)}
              </span>
              <span className="text-gray-500 ml-2 text-sm">credits</span>
            </div>
          )}
          <button
        ...
        {/* Usage Summary */}
        <div className="md:col-span-2 bg-[#111827] border border-gray-800 rounded-2xl p-6 shadow-xl">
          <h2 className="text-lg font-bold text-white mb-5">Usage Summary</h2>
          {loading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="animate-pulse h-16 bg-gray-800 rounded-xl" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                [...Array(3)].map((_, i) => (
                  <tr key={i}>
                    <td colSpan={4} className="px-6 py-4">
                      <div className="animate-pulse h-4 bg-gray-800 rounded w-full" />
                    </td>
                  </tr>
                ))
      ...
      {showTopUpModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-[#111827] border border-gray-800 rounded-2xl w-full max-w-md shadow-2xl p-6">
            <h3 className="text-xl font-bold text-white mb-2">Top Up Credits</h3>
            <p className="text-gray-400 text-sm mb-6">Minimum amount is $5 USD.</p>
            <form onSubmit={handleTopUp}>
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-gray-900 border border-gray-700 rounded-lg pl-8 pr-4 py-3 text-white focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition font-mono"
                    autoFocus
                  />
                </div>
              </div>
              <div className="flex gap-2 mb-6">
                {[10, 25, 50, 100].map(amt => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded text-sm transition border ${topUpAmount === amt ? 'bg-orange-500 border-orange-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-700'}`}
                  >
                    ${amt}
                  </button>
                ))}
              ...
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setShowTopUpModal(false)} className="px-4 py-2 text-gray-400 hover:text-white transition" disabled={isProcessing}>
                  Cancel
                </button>
                <button
                  type="submit"
