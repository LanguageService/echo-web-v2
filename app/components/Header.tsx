    ...
    <header className="bg-white dark:bg-gray-900 border-b dark:border-gray-700">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2 font-semibold text-lg text-gray-900 dark:text-white">
          <div className="w-9 h-9 rounded-full bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center text-orange-600">
            文A
          </div>
          Lingua<span className="text-green-500">Flow</span>
        </div>

        <nav className="flex items-center gap-8 text-sm font-medium text-gray-500 dark:text-gray-400">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            ...
            <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full border border-white dark:border-gray-900" />
          </div>
        </div>
      </div>
    </header>
...
function NavItem({ icon, label, active = false }: { icon: React.ReactNode; label: string; active?: boolean }) {
  return (
    <div className={`flex items-center gap-2 cursor-pointer pb-1 ${active ? "text-orange-500 border-b-2 border-orange-500" : "hover:text-gray-700 dark:hover:text-gray-200"
      }`}>
      {icon}
      {label}
    </div>
